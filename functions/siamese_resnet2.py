...
class SiameseInceptionResnetV1(nn.Module):
    ...
    """
    def __init__(self, pretrained=None, classify=False, num_classes=None, num_out_comp=None, dropout_prob=0.6, device=None):
        super().__init__()
        ...
        # elif pretrained is None and not self.classify and self.num_out_comp is None:
        #     raise Exception('If "pretrained" is not specified and "classify" is False, "num_out_comp" must be specified')
        ...
        #     nn.Sigmoid(),
        # )
        self.sameclass = nn.Sequential(nn.Linear(out_features, 1), nn.Sigmoid())
    ...
    def forward(self, img1, img2):
        ...
        dis = torch.abs(feat1 - feat2)
        output = self.sameclass(dis)
        # output = self.out(same)
